...
function SearchResults({ searchQuery }: { searchQuery: string }) {
  ...
  if (searchQuery === "") {
    return <p className="text-muted-foreground text-center mt-4">Digite um termo para iniciar a busca por pesquisadores.</p>;
  }
  ...
  if (researchersError) {
    return <div className="text-center mt-4 text-red-600">Erro ao carregar pesquisadores: {researchersError.message}</div>;
  }

  const researchersArray: OrcidExpandedResultItem[] = getArrayFromData(data, "expanded-result");
  ...
  return (
    <div className="mt-8">
      ...
      {researchersArray.length === 0 && !isLoading && (
        <p className="text-center mt-4">Nenhum pesquisador encontrado para "{searchQuery}".</p>
      )}
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6 text-center">Busca de Pesquisadores</h1>
