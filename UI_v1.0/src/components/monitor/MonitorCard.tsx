...
export default function MonitorCard({
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useState(initialNotificationState);
  
  const toggleNotifications = () => {
    setNotificationsEnabled(!notificationsEnabled);
    toast({
      title: notificationsEnabled ? "Notificações desativadas" : "Notificações ativadas",
      description: `As notificações para ${name} foram ${notificationsEnabled ? "desativadas" : "ativadas"}.`,
      duration: 3000,
    });
  };
      ...
      <CardContent className="p-5">
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <Link to={`/profile/${id}`} className="font-medium hover:text-primary">
                {name}
              </Link>
          ...
          {recentChanges.length > 0 ? (
            <div className="space-y-2">
              {recentChanges.map((change, index) => (
                <div key={index} className="bg-secondary/50 rounded-md p-2 text-sm">
                  <Badge variant="outline" className="mb-1">
                    {change.type === "publication" ? "Nova publicação" : "Novas citações"}
                  </Badge>
